 ...
 * In Wails mode, routes through Go backend to avoid CORS issues with local providers.
 */
...
/**
 * Call FetchModelsAPI through Wails Go backend
 */
async function wailsFetchModels(baseUrl: string, apiKey: string): Promise<string> {
  // Try dynamic import of Wails binding first
  try {
    const wailsApp: any = await import("@/../../wailsjs/go/main/App");
    if (wailsApp.FetchModelsAPI) {
      return await wailsApp.FetchModelsAPI(baseUrl, apiKey);
    }
  } catch {
  ...
  if (app?.FetchModelsAPI) {
    return await app.FetchModelsAPI(baseUrl, apiKey);
  }
...
export async function fetchProviderModels(
  ...
  // In Wails mode, route through Go backend to avoid CORS issues
  if (isWailsMode()) {
    const apiKeyToSend = (!apiKey || apiKey === "local-no-key") ? "" : apiKey;
    try {
      const result = await wailsFetchModels(baseUrl, apiKeyToSend);
      const data = JSON.parse(result);
      return parseModelList(data);
    } catch (e) {
      // If Wails backend fails, fall through to direct fetch
      console.warn("[modelFetcher] Wails backend fetch failed, falling back to direct fetch:", e);
    }
  }
